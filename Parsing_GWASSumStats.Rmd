...
DT::datatable(head(gwas_sumstats), caption = "Top of GWAS Summary Statistics for LocusZoom.") 
```
...
```{r}
library(openxlsx)
variant_list <- read.xlsx(paste0(SNP_loc, "/Variants.xlsx"), sheet = "TopLoci")
...
for(VARIANT in variants_of_interest){
  ...
  tempCHR <- subset(variant_list, rsID == VARIANT)[,4]
  tempSTART <- subset(variant_list, rsID == VARIANT)[,16]
  tempEND <- subset(variant_list, rsID == VARIANT)[,17]
  ...
  p1 <- RACER::singlePlotRACER(assoc_data = temp_f_ld, 
  ...
  ggsave(filename = paste0(RACER_loc, "/", Today, ".",VARIANT,".regional_assoc.png"), plot = last_plot())
  ggsave(filename = paste0(RACER_loc, "/", Today, ".",VARIANT,".regional_assoc.pdf"), plot = last_plot())
