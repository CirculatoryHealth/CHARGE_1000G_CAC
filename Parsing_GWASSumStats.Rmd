...
We need to parse and sort the 1000G GWAS CAC data for sharing summary statistics and use in [my.locuszoom.org](https://my.locuszoom.org).
...
gwas_sumstatsLZ <- fread(paste0(GWAS_loc,"/1000G_CAC_EA_AA_filtered_71521.forMyLocusZoom.txt.gz"),
                         showProgress = TRUE)

```
```{r}

gwas_sumstatsALL <- fread(paste0(GWAS_loc,"/1000G_CAC_EA_AA_filtered_71521.organized.txt.gz"),
                         showProgress = TRUE)
```
...
DT::datatable(head(gwas_sumstatsLZ), caption = "Top of GWAS Summary Statistics for LocusZoom.") 
...
summary(gwas_sumstatsLZ)
```

```{r}
cat(paste0("\nTotal number of variants: "), format(length(gwas_sumstatsLZ$chr_vcf), big.mark = ","),".\n")

cat(paste0("\nAvailable chromosomes: "), format(unique(gwas_sumstatsLZ$chr_vcf), big.mark = ","),".\n")

cat("\n\nEffect allele frequency summary: \n")
summary(gwas_sumstatsLZ$Freq1)


cat("\nEffect size summary: \n")
summary(gwas_sumstatsLZ$Effect)
...
Sorting the summary statistics.
```{r}

gwas_sumstatsLZa <- dplyr::arrange(gwas_sumstatsLZ, chr_vcf, position_vcf)

head(gwas_sumstatsLZa)

```

```{r}
fwrite(gwas_sumstatsLZa, 
       paste0(GWAS_loc,"/1000G_CAC_EA_AA_filtered_71521.forMyLocusZoom.sort.txt"),
       quote = FALSE, sep = "\t",
...
```{r Saving}
rm(gwas_sumstatsLZ)
save.image(paste0(PROJECT_loc, "/",Today,".",PROJECTNAME,".GWASSumStats.RData"))
```
