...
RESULTS = paste0(ROOT_loc, "/PLINK/analyses/lookups/AE_20200512_COL_MKAVOUSI_MBOS_CHARGE_1000G_CAC/scRNAseq")
RAWDATA = paste0(ROOT_loc, "/PLINK/_AE_ORIGINALS/AESCRNA/prepped_data")
PROJECT_loc = paste0(ROOT_loc, "/PLINK/analyses/lookups/AE_20200512_COL_MKAVOUSI_MBOS_CHARGE_1000G_CAC/scRNAseq")
...
We check whether the targets genes, _`r CAC_target_genes`_, were sequenced using our method (STARseq). 
...
CAC_target_genes_rm <- c("AC011294.3", "C9orf53", "RP11-145E5.5", "AL137026.1", "ZNF32", "BCAM", "PVRL2", "C6orf195")

temp = CAC_target_genes[!CAC_target_genes %in% CAC_target_genes_rm]

CAC_target_genes_qc <- c(temp, "NECTIN2", "LINC01600")

# VlnPlot(scRNAseqData, features = "LINC01600")

for (GENE in CAC_target_genes_qc){
...
p1 <- DotPlot(scRNAseqData, features = CAC_target_genes_qc,
        cols = "RdBu")
...
FeaturePlot(scRNAseqData, features = c(CAC_target_genes_qc),
...
temp <- MAC.markers[MAC.markers$Gene %in% CAC_target_genes_qc,]
...
temp <- SMC.markers[SMC.markers$Gene %in% CAC_target_genes_qc,]
...
temp <- EC.markers[EC.markers$Gene %in% CAC_target_genes_qc,]
...
temp <- TC.markers[TC.markers$Gene %in% CAC_target_genes_qc,]
...
temp <- BC.markers[BC.markers$Gene %in% CAC_target_genes_qc,]
...
temp <- MC.markers[MC.markers$Gene %in% CAC_target_genes_qc,]
...
temp <- NK.markers[NK.markers$Gene %in% CAC_target_genes_qc,]
...
temp <- MIXED.markers[MIXED.markers$Gene %in% CAC_target_genes_qc,]
