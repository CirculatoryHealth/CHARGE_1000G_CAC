...
AERNA_loc = paste0(ROOT_loc, "/PLINK/_AE_ORIGINALS/AERNA/QC")
...
install.packages.auto("CEMiTool")
...
study_samples_bulkNEW <- colnames(bulkRNA_counts[, -(1:6)])
...
counts <- as.data.frame(bulkRNA_countsFilt[,-(1:6)])  ## assay part
...
cat("  > meta data\n")
temp_coldat <- data.frame(STUDY_NUMBER = names(bulkRNA_countsFilt[,7:628]), 
                          SampleType = "plaque", RNAseqType = "3' RNAseq", 
                          row.names = names(bulkRNA_countsFilt[,7:628]))
cat("  > clinical data\n")
...
rm(temp, temp_coldat, temp_coldat_clin)
...
target_genes_rm <- c("AC011294.3", "C9orf53", "RP11-145E5.5", "AL137026.1", "PVRL2", "C6orf195", "DUPD1")
...
# for debug
target_genes_qc_test <- c("APOE", "ADAMTS7", "CHRNA5", "MORF4L1")
...
## Compare expression to the expression of a sample of 10,000 genes

**Figure 3: comparing expression of genes of interest to mean expression of a sample of 10,000 random genes**
...
expression_long2 <- expression_wide2 %>%
  ...
  mutate(gene = str_replace_all(gene, c("expression_value_sample" = "Mean of 10,000 genes"))) %>%
  mutate(gene = factor(gene, levels = c("Mean of 10,000 genes", target_genes_qc)))
...
# 
  ...
  p1
  ggsave(filename = paste0(PLOT_loc, "/", Today, ".TargetExpression_vs_10000genes.png"), plot = last_plot())
  ggsave(filename = paste0(PLOT_loc, "/", Today, ".TargetExpression_vs_10000genes.pdf"), plot = last_plot())
  rm(p1 )
...
temp <- expression_wide %>%
  ...
  dplyr::select(plaque_phenotypes_cat, plaque_phenotypes_num, epmajor.30days, epmajor.3years, target_genes_qc) %>% 
  drop_na() %>% # drop NA 
...
ggsave(filename = paste0(PLOT_loc, "/", Today, ".correlations.pdf"), plot = p1)

ggsave(filename = paste0(PLOT_loc, "/", Today, ".correlations.png"), plot = p1)

rm(temp, temp.cor, p1)

```

**Figure 8: correlation heatmap between expression of genes of interest**
```{r heatmap_corr_genes, fig.height = 6, fig.width = 7}
library(tidyverse)
library(magrittr)
...
ggsave(filename = paste0(PLOT_loc, "/", Today, ".correlations.target_genes.pdf"), plot = p1)

ggsave(filename = paste0(PLOT_loc, "/", Today, ".correlations.target_genes.png"), plot = p1)
...
**Figure 9: genes of interest plotted over plaque vulnerability index**
```{r boxplot_function, plaque vulnerability index, fig.width = 12, fig.height = 10}
# Make directory for plots
ifelse(!dir.exists(file.path(QC_loc, "/PlaqueVulnerabilityIndex")), 
       dir.create(file.path(QC_loc, "/PlaqueVulnerabilityIndex")), 
       FALSE)
PVI_loc = paste0(QC_loc,"/PlaqueVulnerabilityIndex")
...
for(GENE in target_genes_qc){
  ...
  ggsave(filename = paste0(PVI_loc, "/", Today, ".",GENE,".expression_vs_plaquevulnerabilityindex.sex.png"), plot = p2)
  ggsave(filename = paste0(PVI_loc, "/", Today, ".",GENE,".expression_vs_plaquevulnerabilityindex.sex.pdf"), plot = p2)
...
**Figure 10: correlation heatmap between plaque vulnerability index, and genes of interest**
```{r heatmap_corr, PVI, fig.height = 6, fig.width = 7}
library(tidyverse)
library(magrittr)
...
ggsave(filename = paste0(PLOT_loc, "/", Today, ".correlations.PVI.pdf"), plot = p1)

ggsave(filename = paste0(PLOT_loc, "/", Today, ".correlations.PVI.png"), plot = p1)

rm(temp, temp.cor, p1)

```

```{r kruskal_wallis_all, PVI}
